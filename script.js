    ...
    const fetchActors = async () => {
      ...
      for (let i = 0; i < actorNames.results.length; i++) {
        ...
        if (actorNames.results[i].profile_path === null) {
          ...
          actorImage.src = `${
            BACKDROP_BASE_URL + actorNames.results[i].profile_path
          }`;
        }
      ...
      for (let s = 0; s < movies.results.length; s++) {
        for (let i = 0; i < movies.results[s].genre_ids.length; i++) {
          ...
          if (Number(genres.value) === movies.results[s].genre_ids[i]) {
            ...
            if (
              movies.results[s].backdrop_path === null ||
              movies.results[s].profile_path === null
            ) {
              continue;
            } else {
              if (movies.results[s].backdrop_path) {
                movieImage.src = `${
                  BACKDROP_BASE_URL + movies.results[s].backdrop_path
                }`;
              } else {
                movieImage.src = `${
                  BACKDROP_BASE_URL + movies.results[s].profile_path
                }`;
              }
...
filterButton.addEventListener("change", function () {
  ...
  const fetchMovies = async () => {
    ...
    for (let i = 0; i < movieList.results.length; i++) {
      ...
      movieImage.src = `${
        BACKDROP_BASE_URL + movieList.results[i].backdrop_path
      }`;
...
search.addEventListener("input", function () {
  ...
  const fetchSearchMovies = async () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (
        movies.results[i].backdrop_path === null ||
        movies.results[i].profile_path === null
      ) {
        continue;
      } else {
        if (movies.results[i].backdrop_path) {
          movieImage.src = `${
            BACKDROP_BASE_URL + movies.results[i].backdrop_path
          }`;
        } else {
          movieImage.src = `${
            BACKDROP_BASE_URL + movies.results[i].profile_path
          }`;
        }
